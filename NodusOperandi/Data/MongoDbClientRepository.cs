...
namespace NodusOperandi.Data
{
    ...
    public class MongoDbClientRepository : IClientRepository
    {
        ...
        {
            this.collection = database.GetCollection<ClientModel>("NodusOperandiClients");
        }
        ...
        {
            return this.collection.FindOne(Query<ClientModel>.Where(client => client.MacAddress == macAddress));
        }

        public ClientModel GetByIpv4Address(string ipv4Address)
        {
            return this.collection.FindOne(Query<ClientModel>.Where(client => client.Ipv4Address == ipv4Address));
        }

        public ClientModel GetByIpv6Address(string ipv6Address)
        {
            return this.collection.FindOne(Query<ClientModel>.Where(client => client.Ipv6Address == ipv6Address));
        }
        ...
        {
            return this.collection.Find(Query<ClientModel>.Where(client => client.IsActive && !client.IsDeleted && client.LastPingStatus == IPStatus.Success)).Count();
        }
        ...
        {
            return goByUptime
                    ? this.collection.Find(Query<ClientModel>.Where(client => client.IsActive && !client.IsDeleted && client.UptimeStartedAt != null))
                    : this.collection.Find(Query<ClientModel>.Where(client => client.IsActive && !client.IsDeleted && client.LastPingStatus == IPStatus.Success));
        }
